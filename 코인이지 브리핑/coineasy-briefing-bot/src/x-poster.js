...
// X에 트윗 포스팅 (미디어 첨부 지원)
// ============================================================
export async function postToX(text, mediaId = null) {
      try {
              const client = getXClient();
              const rwClient = client.readWrite;

        console.log(`[X 포스팅] 트윗 발송 중... (${text.length}자)${mediaId ? ' + 배너 이미지' : ''}`);
        ...
        const tweet = await rwClient.v2.tweet(text, tweetOptions);
