...
// 프레임 크기: 1082 x 1343
// 좌표는 Figma에서 각 텍스트 노드의 X, Y, W, H를 직접 확인한 값
// ============================================================
const TEXT_OVERLAYS = [
       // ── BTC 메인 가격 ($69,671) ──
     {
              ...
              x: 70, y: 240, w: 225, h: 67,
              bg: '#1C3A2A', font: 'black 56px', color: '#FFFFFF', align: 'left',
     },
       // ── BTC 라벨 + 변동률 (BTC 텍스트 위치에 심볼+변동률) ──
     {
              key: 'btcChange',
              format: (_, data) => {
                         const btc = findCoin(data, 'BTC');
              ...
              x: 70, y: 213, w: 200, h: 26,
              bg: '#1C3A2A', font: 'medium 22px', color: '#FFFFFF', align: 'left',
     },
       // ── MARKET 전체 변동률 (우측 큰 글씨 +3.27%) ──
     {
              key: 'marketChange',
              format: (_, data) => {
              ...
              x: 788, y: 240, w: 182, h: 58,
              bg: '#1C3A2A', font: 'black 52px', color: '#22C55E', align: 'left',
     },
       // ── MARKET 라벨 (MARKET +2.7%) ──
     {
              key: 'marketLabel',
              format: (_, data) => {
                         const c = parseFloat(data?.global?.marketCapChange24h || 0);
              ...
              x: 841, y: 213, w: 129, h: 22,
              bg: '#1C3A2A', font: 'bold 14px', color: '#AAAAAA', align: 'right',
     },
       // ── 날짜 텍스트 (4월 7일 화요일 아침) ──
     {
              key: 'dateText',
              ...
              x: 449, y: 135, w: 183, h: 26,
              bg: '#3F2912', font: 'bold 22px', color: '#FFFFFF', align: 'center',
     },
       // ── ETH 가격 ──
     {
              key: 'ethPrice',
              ...
              x: 70, y: 398, w: 160, h: 34,
              bg: '#FFFFFF', font: 'black 28px', color: '#1a1a1a', align: 'left',
     },
       // ── ETH 변동률 ──
     {
              key: 'ethChange',
              format: (_, data) => {
                         const eth = findCoin(data, 'ETH');
                         if (!eth) return '';
                         return fmtChange(eth.change24h);
              },
              x: 70, y: 438, w: 160, h: 24,
              bg: '#FFFFFF', font: 'bold 20px', color: '#22C55E', align: 'left',
     },
       // ── SOL 가격 ──
     {
              key: 'solPrice',
       ...
       // ── SOL 변동률 ──
     {
              ...
              format: (_, data) => {
                         const sol = findCoin(data, 'SOL');
                         if (!sol) return '';
                         return fmtChange(sol.change24h);
              },
              x: 318, y: 438, w: 160, h: 24,
              bg: '#FFFFFF', font: 'bold 20px', color: '#22C55E', align: 'left',
     },
       // ── SUI 가격 ──
     {
       ...
       // ── SUI 변동률 ──
     {
              ...
              format: (_, data) => {
                         const sui = findCoin(data, 'SUI');
                         if (!sui) return '';
                         return fmtChange(sui.change24h);
              },
              x: 566, y: 438, w: 160, h: 24,
              bg: '#FFFFFF', font: 'bold 20px', color: '#22C55E', align: 'left',
     },
       // ── XRP 가격 ──
     {
       ...
       // ── XRP 변동률 ──
     {
              ...
              format: (_, data) => {
                         const xrp = findCoin(data, 'XRP');
                         if (!xrp) return '';
                         return fmtChange(xrp.change24h);
              },
              x: 814, y: 438, w: 160, h: 24,
              bg: '#FFFFFF', font: 'bold 20px', color: '#22C55E', align: 'left',
     },
       // ── 김프 환율 (환율: ₩1,507/USDT) ──
     {
       ...
       // ── 김프 프리미엄 (프리미엄: 0.03%) ──
     {
              ...
              x: 64, y: 610, w: 350, h: 29,
              bg: '#FFFFFF', font: 'bold 22px', color: '#FF6B17', align: 'left',
     },
       // ── 공포/탐욕 지수 숫자 (38) ──
     {
              key: 'fearGreedValue',
              format: (_, data) => String(data?.fearGreed?.value ?? '--'),
              x: 582, y: 605, w: 55, h: 46,
              bg: '#FFFFFF', font: 'black 38px', color: '#333333', align: 'left',
     },
       // ── 공포/탐욕 라벨 (Fear) ──
     {
              key: 'fearGreedLabel',
              format: (_, data) => data?.fearGreed?.label || '--',
              x: 641, y: 620, w: 100, h: 24,
              bg: '#FFFFFF', font: 'bold 20px', color: '#666666', align: 'left',
     },
     ];
...
// Figma XY = 텍스트 노드 좌상단 기준 → canvas에서 textBaseline='top' 사용
// ============================================================
function overlayTexts(canvas, ctx, data, scale) {
       ...
       for (const ov of TEXT_OVERLAYS) {
                try {
                  ...
                  // 1) 배경색으로 기존 텍스트 영역 덮기 (Figma XY가 좌상단이므로 그대로)
                  ctx.fillStyle = ov.bg;
                  ...
                  let textColor = ov.color;
                           if (ov.key.endsWith('Change') && value.startsWith('-')) {
                                        textColor = '#EF4444';
                           }
                           ctx.fillStyle = textColor;

                  // 4) 텍스트 baseline=top으로 그리기 (Figma Y=상단)
                  ctx.textBaseline = 'top';
                           const padding = 2 * scale; // 상단 여백 약간
