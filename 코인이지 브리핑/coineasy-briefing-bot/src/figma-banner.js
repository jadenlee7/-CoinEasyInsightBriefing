...
const TEXT_OVERLAYS = [
         ...
         // ── 공포/탐욕 라벨 ──
       {
         ...
         // ══════════════════════════════════════════════════════
         // DeFi 핫이슈 섹션 — 3개 항목 동적 오버레이
         // bg: '#F7F4EE' (DeFi 카드 배경)
         // ══════════════════════════════════════════════════════
         ...
         // ── DeFi #1 이름 ──
       {
                  ...
                  x: 86, y: 778, w: 200, h: 28, bg: '#F7F4EE',
                  font: 'black 22px', color: '#1a1a1a', align: 'left',
       },
         // ── DeFi #1 설명 ──
       {
                  key: 'defi1Desc',
                  format: (_, data) => {
                               const items = getDefiItems(data);
                               if (!items[0]) return '';
                               return `TVL ${items[0].tvl}  ${items[0].category}`;
                  },
                  x: 86, y: 784, w: 450, h: 22, bg: '#F7F4EE',
                  font: 'medium 18px', color: '#888888', align: 'left',
       },
         // ── DeFi #1 변동률 ──
       {
                  key: 'defi1Change',
         ...
         // ── DeFi #2 이름 ──
       {
                  ...
                  x: 86, y: 820, w: 350, h: 28, bg: '#F7F4EE',
                  font: 'black 22px', color: '#1a1a1a', align: 'left',
       },
         // ── DeFi #2 설명 ──
       {
                  ...
                  format: (_, data) => {
                               const items = getDefiItems(data);
                               if (!items[1]) return '';
                               return `TVL ${items[1].tvl}  ${items[1].category}`;
                  },
                  x: 86, y: 826, w: 450, h: 22, bg: '#F7F4EE',
                  font: 'medium 18px', color: '#888888', align: 'left',
       },
         // ── DeFi #2 변동률 ──
       {
                  ...
                  x: 900, y: 821, w: 100, h: 28, bg: '#F7F4EE',
                  font: 'black 22px', color: '#22C55E', align: 'right',
       },
         // ── DeFi #3 이름 ──
       {
                  ...
                  x: 86, y: 862, w: 200, h: 28, bg: '#F7F4EE',
                  font: 'black 22px', color: '#1a1a1a', align: 'left',
       },
         // ── DeFi #3 설명 ──
       {
                  ...
                  format: (_, data) => {
                               const items = getDefiItems(data);
                               if (!items[2]) return '';
                               return `TVL ${items[2].tvl}  ${items[2].category}`;
                  },
                  x: 86, y: 868, w: 450, h: 22, bg: '#F7F4EE',
                  font: 'medium 18px', color: '#888888', align: 'left',
       },
         // ── DeFi #3 변동률 ──
       {
...
export async function exportFigmaBanner(data) {
  ...
  try {
           ...
           if (data) {
                        ...
                        console.log('  📊 data.trending 수:', data.trending?.length || 0);
                        if (data.trending && Array.isArray(data.trending)) {
                                       data.trending.slice(0, 3).forEach((t, i) => console.log(`    - 트렌딩 ${i+1}: ${t.symbol} (${t.name}) ${t.priceChange24h}%`));
                        }
                        console.log('  📊 data.defi:', data.defi ? 'available' : 'null');
