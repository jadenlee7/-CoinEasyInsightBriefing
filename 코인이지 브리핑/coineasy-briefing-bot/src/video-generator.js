/**
 * 코인이지 YouTube Shorts 영상 생성 모듈 v3.1
 * 인포그래픽 카드 슬라이드쇼 + Edge TTS + FFmpeg
 * 9:16 세로 포맷, 30~45초 쇼츠 생성
 *
 * v3.1 수정사항:
 * - 날짜를 KST(한국시간) 기준으로 표시
 * - formatKrw NaN 방어 강화
 * - 김치 프리미엄 NaN 방어
 */
...
function getKstDateDot() {
       return getKstDateStr().replace(/-/g, '.');
}
...
// ============================================================
            // 한국식 원화 포맷 (105391000 -> "1억 539만원")
// ============================================================
function formatKrw(numStr) {
       if (!numStr && numStr !== 0) return '0원';
       const cleaned = typeof numStr === 'string' ? numStr.replace(/,/g, '') : String(numStr);
...
// ============================================================
// 카드별 FFmpeg drawtext 필터 생성 (v3.1)
// ============================================================
...
export async function createShortsVideo(script, marketData) {
  ...
  const dateStr = getKstDateStr();
       console.log(`[쇼츠 생성] KST 날짜: ${dateStr}`);
...
function buildCardsFromData(script, data, dateStr) {
  ...
  const krwPriceStr = kimchi?.upbitBtcKrw ? formatKrw(kimchi.upbitBtcKrw) : '';
       console.log(`[카드 데이터] 업비트 원본: ${kimchi?.upbitBtcKrw}, 변환: ${krwPriceStr}`);
  ...
  const fg = data?.fearGreed;
       const kpValue = kimchi?.premium || '0';
       const kimchiKrwStr = kimchi?.upbitBtcKrw ? `업비트 BTC ${formatKrw(kimchi.upbitBtcKrw)}` : '';
       const krwRateStr = kimchi?.krwRate ? `환율 ${kimchi.krwRate}원/USDT` : '';

  console.log(`[카드 데이터] 김프: ${kpValue}%%, 업비트: ${kimchiKrwStr}`);
  ...
  cards.push({
           ...
           kimchiPremium: kpValue,
           kimchiDetail: kimchiKrwStr,
           krwRate: krwRateStr,
